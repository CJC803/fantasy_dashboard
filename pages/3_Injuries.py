    ...
    if injuries.empty:
        ...
        if team_col:
            ...
            with col1:
                ...
                main_color = blues_palette[2]  # mid-tone blue
            
                # convert to RGBA safely
                if main_color.startswith("#"):
                    r = int(main_color[1:3], 16)
                    g = int(main_color[3:5], 16)
                    b = int(main_color[5:7], 16)
                elif main_color.startswith("rgb"):
                    nums = [int(x) for x in main_color.strip("rgb() ").split(",")]
                    r, g, b = nums[:3]
                else:
                    # fallback: default blue tone
                    r, g, b = (30, 144, 255)
            
                fill_rgba = f"rgba({r},{g},{b},0.5)"
