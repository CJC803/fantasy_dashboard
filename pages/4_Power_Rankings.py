...
with st.expander("🔎 Extra insights"):
    # Highest All-Play %
    ap_idx = power["All-Play %"].idxmax()
    ap_team = power.loc[ap_idx, "Team"]
    ap_val = power.loc[ap_idx, "All-Play %"]

    # Hardest SoS (higher opp PF avg)
    sos_idx = power["SoS (opp PF avg)"].idxmax()
    sos_team = power.loc[sos_idx, "Team"]
    sos_val = power.loc[sos_idx, "SoS (opp PF avg)"]
